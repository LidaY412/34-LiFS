...
class Dataset_for_external_validation(Dataset):
    ...
    Loads 3-modal 3D medical images from NIfTI files for validation tasks
    """
    ...
    def __getitem__(self, idx):
        ...
        # Read 3 modal images from NIfTI files
        ...
        # Concatenate 3 modalities along the last axis (channel dimension)
...
if args.model_path is None:
    ...
    args.model_path = f'./noncontrast_trained_models/trained_models_{args.task}/bs{args.bs}_epoch{args.epoch}_seed{args.seed}/best_ACC_val.pth'

# Auto-generate save directory if not specified (task-specific)
if args.save_dir is None:
    args.save_dir = f'./noncontrast_results/val_results/val_results_{args.task}'
...
# Initialize 3D ResNet model with 3-stream LSTM (binary classification)
...
with torch.no_grad():
    for i, (T1W1_imgs, T2W2_imgs, DWI_imgs, case_ids) in enumerate(
            exval_dataloader):
        ...
        for j, case_id in enumerate(case_ids):
            predictions.append({
                'Case': case_id,
                'Setting': 'NonContrast',  # Modify this if S4 requires a different setting
